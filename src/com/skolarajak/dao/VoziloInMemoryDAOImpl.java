...
public class VoziloInMemoryDAOImpl implements VoziloDAO {
	...
	public List<Vozilo> getAll() throws ResultNotFoundException {
		...
		return VoziloInMemoryDAOImpl.registrovanaVozila.values().stream().collect(Collectors.toList());
	}

	@Override
	public List<Vozilo> getEuro3Vozila() {
		// TODO Auto-generated method stub
		return VoziloInMemoryDAOImpl.registrovanaVozila.values().stream().filter(v -> v.getGodisteProizvodnje() >= 2000)
	...
	public List<Vozilo> getAktivnaVozila() {
		return VoziloInMemoryDAOImpl.registrovanaVozila.values().stream().filter(v -> v.isAktivno()).collect(Collectors.toList());
	}
